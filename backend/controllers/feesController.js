...
exports.createFeeHead = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error creating fee head", error: error.message });
  }
};
...
exports.getFeeHeads = async (req, res) => {
  try {
    const [heads] = await db.query(
      "SELECT * FROM fee_heads WHERE is_active = true"
    );
    ...
    res
      .status(500)
      .json({ message: "Error fetching fee heads", error: error.message });
  }
};
...
exports.createFeeStructure = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Fee structure added", id: result.insertId });
  } catch (error) {
    console.error("Error adding fee structure:", error);
    res
      .status(500)
      .json({ message: "Error adding fee structure", error: error.message });
  }
};
...
exports.getFeeStructure = async (req, res) => {
  try {
    ...
    if (userRole === "student") {
      // Students: Only see fee structures for their assigned class
    ...
    // Teachers and Admin/Principal/Higher roles: See all fee structures (no additional filter)
...
exports.getStudentFeeStatus = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error fetching student fee status",
        error: error.message,
      });
  }
};
