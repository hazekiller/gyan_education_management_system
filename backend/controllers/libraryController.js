...
const addBook = async (req, res) => {
    try {
        const {
            book_title, author, isbn, publisher, publication_year,
            category, total_copies, rack_number, description
        } = req.body;

        if (!book_title || !total_copies) {
            return res.status(400).json({ success: false, message: 'Title and Total Copies are required' });
        }

        // Check ISBN uniqueness if provided
        if (isbn) {
            const [existing] = await pool.query('SELECT id FROM library_books WHERE isbn = ?', [isbn]);
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                book_title, author, isbn, publisher, publication_year,
                category, total_copies, total_copies, // available = total initially
...
const updateBook = async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE library_books SET ${setClause} WHERE id = ?`, values);
...
const issueBook = async (req, res) => {
    ...
    try {
        ...
        if (user_type === 'student') {
            const [s] = await connection.query('SELECT user_id FROM students WHERE id = ?', [user_id]);
            if (s.length === 0) throw new Error('Student not found');
            dbUserId = s[0].user_id;
        } else if (user_type === 'teacher') {
            const [t] = await connection.query('SELECT user_id FROM teachers WHERE id = ?', [user_id]);
        ...
        const [books] = await connection.query('SELECT available_copies, total_copies FROM library_books WHERE id = ?', [book_id]);
        ...
        if (existing.length > 0) throw new Error('User already has this book issued');
      ...
      VALUES (?, ?, ?, CURDATE(), ?, 'issued', ?, ?)`,
        ...
        res.status(400).json({ success: false, message: error.message || 'Failed to issue book' });
    } finally {
        connection.release();
    }
};
...
const returnBook = async (req, res) => {
    ...
    try {
        ...
        const { transaction_id, return_date, fine_amount, remarks, status } = req.body;
        // status can be 'returned' or 'lost'

        // 1. Get transaction details
        const [trans] = await connection.query('SELECT book_id, status FROM library_transactions WHERE id = ?', [transaction_id]);
        if (trans.length === 0) throw new Error('Transaction not found');
        if (trans[0].status !== 'issued' && trans[0].status !== 'overdue') throw new Error('Book already returned or processed');
       ...
       WHERE id = ?`,
            [return_date || new Date(), fine_amount || 0, finalStatus, remarks, transaction_id]
        );
        ...
        res.status(400).json({ success: false, message: error.message || 'Failed to return book' });
    } finally {
        connection.release();
    }
};
...
const getMyBooks = async (req, res) => {
    ...
    `, [userId]);
        ...
        res.status(500).json({ success: false, message: 'Failed to fetch your books', error: error.message });
    }
};
