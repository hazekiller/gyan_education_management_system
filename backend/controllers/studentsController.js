...
const createStudent = async (req, res) => {
  ...
  try {
    ...
    if (!admission_number || !first_name || !last_name || !email || !date_of_birth || !gender || !class_id || !parent_phone) {
      await connection.rollback();
      return res.status(400).json({
        success: false,
        message: 'Missing required fields: admission_number, first_name, last_name, email, date_of_birth, gender, class_id, parent_phone'
      });
    }
    ...
    const [studentResult] = await connection.query(
      ...
      [
        userId, admission_number, first_name, middle_name || null, last_name,
        email, phone || null, date_of_birth, gender, blood_group || null,
        class_id, section_id || null, roll_number || null, admission_date || new Date(),
        father_name || null, mother_name || null, parent_phone, parent_email || null,
