  ...
  try {
    ...
    if (duplicate.length > 0) {
      ...
      return res.status(409).json({
        success: false,
        message: 'This subject is already assigned to this class for the specified academic year'
      });
    }
  ...
  try {
    ...
    const [result] = await connection.query(
      `INSERT INTO section_subject_teachers 
       ...
       VALUES (?, ?, ?, ?, NOW(), NOW())`,
      [section_id, subject_id, teacher_id, academic_year || new Date().getFullYear()]
    );
...
const removeSubjectFromClass = async (req, res) => {
  ...
  try {
    ...
    if (timetableUsage[0].count > 0) {
      ...
      return res.status(400).json({
        success: false,
        message: 'Cannot remove subject. It is being used in timetable. Please remove from timetable first.'
      });
    }
