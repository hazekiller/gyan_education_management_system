...
import { Home, Plus, Users, UserMinus, Bed, Building } from 'lucide-react';
...
const HostelManagement = () => {
    ...
    const isAdmin = ['super_admin', 'admin', 'principal', 'vice_principal'].includes(user?.role);
...
const RoomsTab = () => {
    ...
    const { data: rooms = [], isLoading } = useQuery({
        ...
        queryFn: () => hostelAPI.getAllRooms({ type: filterType === 'all' ? undefined : filterType }).then(res => res.data || [])
    });
    ...
    const vacateMutation = useMutation({
        ...
        onError: (err) => toast.error(err.response?.data?.message || 'Failed to vacate')
    });
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {isLoading ? (
                    ...
                    <p className="text-gray-500 col-span-full text-center py-8">No rooms found.</p>
                ) : (
                    rooms.map((room) => (
                        <div key={room.id} className="border rounded-xl p-5 hover:shadow-lg transition-shadow bg-gray-50">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    <h3 className="font-bold text-lg text-gray-800">Room {room.room_number}</h3>
                                    <p className="text-xs text-gray-500">{room.building_name}</p>
                                </div>
                                <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${room.type === 'male' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700'
                                    }`}>
                                    {room.type}
                                </span>
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className={`font-medium ${room.current_occupancy >= room.capacity ? 'text-red-600' : 'text-green-600'}`}>
                                        {room.current_occupancy} / {room.capacity}
                                    </span>
                                </div>
                                <div className="w-full bg-gray-200 rounded-full h-2">
                                    <div
                                        className={`h-2 rounded-full ${room.current_occupancy >= room.capacity ? 'bg-red-500' : 'bg-green-500'}`}
            ...
            {selectedRoom && !showAllocateModal && (
                <RoomDetailsModal
                    ...
                    onVacate={(allocationId) => vacateMutation.mutate({ allocation_id: allocationId })}
                />
            )}
        </div>
    );
...
const MyRoomTab = () => {
    ...
    if (isLoading) return <p className="p-8 text-center">Loading room details...</p>;
    ...
    if (!data) {
        return (
            <div className="text-center py-12 bg-gray-50 rounded-lg border border-dashed">
                ...
                <p className="text-gray-500 mt-2">You have not been allocated a hostel room yet.</p>
            </div>
        );
    }
        ...
        <div className="max-w-4xl mx-auto">
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-6 mb-8">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">Room {allocation.room_number}</h2>
                        <p className="text-blue-600 font-medium">{allocation.building_name}</p>
                    </div>
                    <div className="bg-white px-4 py-2 rounded-lg shadow-sm">
                        <span className="text-sm text-gray-500 block">Allocated Date</span>
                        <span className="font-medium">{new Date(allocation.allocation_date).toLocaleDateString()}</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {partners.length === 0 ? (
                        ...
                        partners.map((partner, idx) => (
                            <div key={idx} className="flex items-center p-4 bg-white border rounded-lg shadow-sm">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-500">Class {partner.class_id}</p>
                                </div>
                            </div>
                        ))
                    )}
...
const AddRoomModal = ({ onClose }) => {
    ...
    const [formData, setFormData] = useState({
        room_number: '', building_name: 'Main Hostel', type: 'male', capacity: 4
    });
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex justify-end space-x-3 mt-6">
                        ...
                        <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Create Room</button>
                    </div>
                </form>
            </div>
        </div>
...
const AllocateRoomModal = ({ room, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
                <h2 className="text-xl font-bold mb-2">Allocate Room {room.room_number}</h2>
                <p className="text-sm text-gray-500 mb-4">Type: {room.type} | Available Spots: {room.capacity - room.current_occupancy}</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        {filteredStudents.length === 0 && (
                            <p className="text-xs text-red-500 mt-1">No students of matching gender found.</p>
                        )}
                    </div>
                    <div className="flex justify-end space-x-3 mt-6">
                        <button type="button" onClick={onClose} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded">Cancel</button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
                <div className="flex justify-between items-start mb-6">
                    <div>
                        <h2 className="text-2xl font-bold">Room {room.room_number} Details</h2>
                        <p className="text-gray-500">{room.building_name} ({room.type})</p>
                    </div>
                ...
                {isLoading ? (
                    ...
                    <div>
                        <h3 className="font-bold text-lg mb-3">Occupants ({roomDetails?.occupants?.length || 0}/{room.capacity})</h3>
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-gray-50">
                                    <tr>
                                        <th className="p-3 text-sm font-medium text-gray-600">Name</th>
                                        <th className="p-3 text-sm font-medium text-gray-600">Admission No</th>
                                        <th className="p-3 text-sm font-medium text-gray-600">Class</th>
                                        <th className="p-3 text-sm font-medium text-gray-600">Allocated Date</th>
                                        <th className="p-3 text-sm font-medium text-gray-600 text-right">Action</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y">
                                    {roomDetails?.occupants?.length === 0 ? (
                                        <tr><td colSpan="5" className="p-4 text-center text-gray-500">Room is empty</td></tr>
                                    ) : (
                                        roomDetails?.occupants?.map((occ) => (
                                            <tr key={occ.id}>
                                                <td className="p-3 font-medium">{occ.first_name} {occ.last_name}</td>
                                                <td className="p-3 text-gray-600">{occ.admission_number}</td>
                                                <td className="p-3 text-gray-600">{occ.class_id}</td>
                                                <td className="p-3 text-gray-600">{new Date(occ.allocation_date).toLocaleDateString()}</td>
