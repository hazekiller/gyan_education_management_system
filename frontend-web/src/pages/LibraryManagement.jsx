...
import { Book, Search, Plus, User, Calendar, CheckCircle, XCircle, History, BookOpen } from 'lucide-react';
...
const LibraryManagement = () => {
    ...
    const isAdmin = ['super_admin', 'principal', 'librarian'].includes(user?.role);

    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-3xl font-bold text-gray-900">Library Management</h1>
                <p className="text-gray-600 mt-1">Manage books, issue/return, and track inventory</p>
            </div>
...
const BooksInventoryTab = ({ isAdmin }) => {
    ...
    const { data: books = [], isLoading } = useQuery({
        ...
        queryFn: () => libraryAPI.getAllBooks({ search }).then(res => res.data || [])
    });
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-gray-50 border-b">
                            ...
                            <th className="p-3 font-medium text-gray-600 text-center">Copies (Avail/Total)</th>
                            {isAdmin && <th className="p-3 font-medium text-gray-600 text-right">Actions</th>}
                        </tr>
                    </thead>
                        ...
                        {isLoading ? (
                            ...
                            books.map((book) => (
                                <tr key={book.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-3 text-gray-600">{book.rack_number || '-'}</td>
...
const AddBookModal = ({ onClose }) => {
    ...
    const [formData, setFormData] = useState({
        book_title: '', author: '', isbn: '', category: '',
        total_copies: 1, rack_number: '', description: ''
    });
...
const IssueReturnTab = () => {
    ...
    const { data: books = [] } = useQuery({
        queryKey: ['books-list'],
        queryFn: () => libraryAPI.getAllBooks({ available_only: true }).then(res => res.data || [])
    });
        ...
        <div className="max-w-2xl mx-auto">
            <div className="flex space-x-4 mb-6 justify-center">
                <button
                    ...
                    className={`px-6 py-2 rounded-full font-medium ${mode === 'issue' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}
                >
                    Issue Book
                </button>
                <button
                    onClick={() => setMode('return')}
                    className={`px-6 py-2 rounded-full font-medium ${mode === 'return' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}
                >
                    Return Book
                </button>
            </div>
            ...
            {mode === 'issue' ? (
                <form onSubmit={handleIssue} className="space-y-6 bg-gray-50 p-8 rounded-xl border">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                ...
                <div className="space-y-6 bg-gray-50 p-8 rounded-xl border">
                    <p className="text-sm text-gray-600 mb-4">Enter the Transaction ID to return a book. You can find this in the History tab.</p>
                    ...
                    {isLoading ? (
                        ...
                        transactions.map((t) => (
                            <tr key={t.id} className="hover:bg-gray-50">
                                ...
                                <td className="p-3 text-gray-600">{new Date(t.issue_date).toLocaleDateString()}</td>
                                <td className="p-3 text-gray-600">{new Date(t.due_date).toLocaleDateString()}</td>
                                <td className="p-3">
                                    <span className={`px-2 py-1 rounded-full text-xs capitalize ${t.status === 'issued' ? 'bg-blue-100 text-blue-800' :
                ...
                myBooks.map((item) => (
                    <div key={item.id} className="border rounded-lg p-5 hover:shadow-md transition-shadow">
                        <div className="flex justify-between items-start mb-3">
                            <h3 className="font-bold text-lg text-gray-800 leading-tight">{item.book_title}</h3>
                            <span className={`px-2 py-1 rounded text-xs font-medium capitalize ${item.status === 'issued' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-600'
                                }`}>
                                {item.status}
                            </span>
                        </div>
                        ...
                        <div className="text-sm space-y-1 border-t pt-3">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className={item.status === 'issued' && new Date(item.due_date) < new Date() ? 'text-red-600 font-bold' : ''}>
