...
import { Bus, MapPin, Users, Plus, Trash2, Navigation, UserCheck } from 'lucide-react';
...
const MyTransportTab = () => {
    ...
    if (isLoading) return <p className="p-8 text-center">Loading transport details...</p>;

    if (!data) {
        return (
            <div className="text-center py-12 bg-gray-50 rounded-lg border border-dashed">
                <Bus className="w-16 h-16 mx-auto mb-4 text-gray-300" />
                <h3 className="text-lg font-medium text-gray-900">No Transport Allocated</h3>
                <p className="text-gray-500 mt-2">You are not subscribed to school transport.</p>
            </div>
        );
    }
        ...
        <div className="max-w-4xl mx-auto">
            <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-6 mb-8">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900">{data.route_name}</h2>
                        <p className="text-yellow-700 font-medium">{data.bus_number} ({data.registration_number})</p>
                    </div>
                    <div className="bg-white px-4 py-2 rounded-lg shadow-sm text-center">
                        ...
                        <span className="text-2xl font-bold text-blue-600">{data.seat_number || 'N/A'}</span>
                    </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div className="bg-white p-4 rounded-lg shadow-sm border-l-4 border-green-500">
                        <p className="text-xs text-gray-500 uppercase font-bold mb-1">Pickup Point</p>
                        <p className="font-bold text-lg">{data.pickup_name}</p>
                        <p className="text-sm text-gray-600">Time: {data.pickup_time?.slice(0, 5)}</p>
                    </div>
                    <div className="bg-white p-4 rounded-lg shadow-sm border-l-4 border-red-500">
                        <p className="text-xs text-gray-500 uppercase font-bold mb-1">Drop Point</p>
                        <p className="font-bold text-lg">{data.drop_name}</p>
                        <p className="text-sm text-gray-600">Time: {data.drop_time?.slice(0, 5)}</p>
                    </div>
                </div>

                <div className="bg-white p-4 rounded-lg shadow-sm">
                    <h4 className="font-bold text-gray-800 mb-3 flex items-center"><UserCheck className="w-4 h-4 mr-2" /> Driver Details</h4>
            ...
            <div className="bg-white rounded-lg p-6 w-full max-w-lg">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <input required placeholder="Bus Number (e.g. Bus-01)" className="border rounded px-3 py-2" value={formData.bus_number} onChange={e => setFormData({ ...formData, bus_number: e.target.value })} />
                        <input required placeholder="Registration No." className="border rounded px-3 py-2" value={formData.registration_number} onChange={e => setFormData({ ...formData, registration_number: e.target.value })} />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <input required placeholder="Driver Name" className="border rounded px-3 py-2" value={formData.driver_name} onChange={e => setFormData({ ...formData, driver_name: e.target.value })} />
                        <input required placeholder="Driver Phone" className="border rounded px-3 py-2" value={formData.driver_phone} onChange={e => setFormData({ ...formData, driver_phone: e.target.value })} />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <input placeholder="Sub-Driver Name" className="border rounded px-3 py-2" value={formData.sub_driver_name} onChange={e => setFormData({ ...formData, sub_driver_name: e.target.value })} />
                        <input placeholder="Sub-Driver Phone" className="border rounded px-3 py-2" value={formData.sub_driver_phone} onChange={e => setFormData({ ...formData, sub_driver_phone: e.target.value })} />
                    </div>
                    <input type="number" placeholder="Capacity" className="w-full border rounded px-3 py-2" value={formData.capacity} onChange={e => setFormData({ ...formData, capacity: e.target.value })} />
...
const AddRouteModal = ({ onClose }) => {
    const queryClient = useQueryClient();
    const [stops, setStops] = useState([{ stop_name: '', pickup_time: '', drop_time: '', fare: 0 }]);
    const [formData, setFormData] = useState({ route_name: '', vehicle_id: '', start_point: '', end_point: '' });

    const { data: vehicles = [] } = useQuery({ queryKey: ['transport-vehicles'], queryFn: () => transportAPI.getAllVehicles().then(res => res.data || []) });

    const mutation = useMutation({
        mutationFn: transportAPI.createRoute,
        onSuccess: () => { queryClient.invalidateQueries(['transport-routes']); toast.success('Route created'); onClose(); },
        onError: (err) => toast.error(err.response?.data?.message || 'Failed')
    });

    const addStop = () => setStops([...stops, { stop_name: '', pickup_time: '', drop_time: '', fare: 0 }]);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <input required placeholder="Route Name" className="border rounded px-3 py-2" value={formData.route_name} onChange={e => setFormData({ ...formData, route_name: e.target.value })} />
                        <select className="border rounded px-3 py-2" value={formData.vehicle_id} onChange={e => setFormData({ ...formData, vehicle_id: e.target.value })}>
                            <option value="">Select Vehicle</option>
                            {vehicles.map(v => <option key={v.id} value={v.id}>{v.bus_number}</option>)}
                        </select>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <input required placeholder="Start Point" className="border rounded px-3 py-2" value={formData.start_point} onChange={e => setFormData({ ...formData, start_point: e.target.value })} />
                        <input required placeholder="End Point" className="border rounded px-3 py-2" value={formData.end_point} onChange={e => setFormData({ ...formData, end_point: e.target.value })} />
                    </div>
                    ...
                    <div className="border-t pt-4">
                        ...
                        {stops.map((stop, idx) => (
                            <div key={idx} className="grid grid-cols-4 gap-2 mb-2">
                                <input placeholder="Stop Name" className="border rounded px-2 py-1 text-sm" value={stop.stop_name} onChange={e => updateStop(idx, 'stop_name', e.target.value)} required />
                                <input type="time" className="border rounded px-2 py-1 text-sm" value={stop.pickup_time} onChange={e => updateStop(idx, 'pickup_time', e.target.value)} required />
                                <input type="time" className="border rounded px-2 py-1 text-sm" value={stop.drop_time} onChange={e => updateStop(idx, 'drop_time', e.target.value)} required />
                                <input type="number" placeholder="Fare" className="border rounded px-2 py-1 text-sm" value={stop.fare} onChange={e => updateStop(idx, 'fare', e.target.value)} />
                            </div>
                        ))}
                        <button type="button" onClick={addStop} className="text-blue-600 text-sm hover:underline">+ Add Stop</button>
                    </div>

                    <div className="flex justify-end space-x-3 mt-4">
                        <button type="button" onClick={onClose} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded">Cancel</button>
                        <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Create Route</button>
                    </div>
                </form>
            </div>
        </div>
...
const AllocateTransportModal = ({ onClose }) => {
    const queryClient = useQueryClient();
    const [formData, setFormData] = useState({ student_id: '', route_id: '', pickup_stop_id: '', drop_stop_id: '', seat_number: '' });

    const { data: students = [] } = useQuery({ queryKey: ['students-list'], queryFn: () => studentsAPI.getAll().then(res => res.data || []) });
    const { data: routes = [] } = useQuery({ queryKey: ['transport-routes'], queryFn: () => transportAPI.getAllRoutes().then(res => res.data || []) });

    const selectedRoute = routes.find(r => r.id == formData.route_id);

    const mutation = useMutation({
        mutationFn: transportAPI.allocateTransport,
        onSuccess: () => { queryClient.invalidateQueries(['transport-allocations']); toast.success('Allocated'); onClose(); },
        onError: (err) => toast.error(err.response?.data?.message || 'Failed')
    });
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
                <h2 className="text-xl font-bold mb-4">Allocate Transport</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <select required className="w-full border rounded px-3 py-2" value={formData.student_id} onChange={e => setFormData({ ...formData, student_id: e.target.value })}>
                        <option value="">Select Student</option>
                        {students.map(s => <option key={s.id} value={s.id}>{s.first_name} {s.last_name} ({s.admission_number})</option>)}
                    </select>

                    <select required className="w-full border rounded px-3 py-2" value={formData.route_id} onChange={e => setFormData({ ...formData, route_id: e.target.value })}>
                    ...
                    {selectedRoute && (
                        <>
                            <div className="grid grid-cols-2 gap-4">
                                <select required className="border rounded px-3 py-2" value={formData.pickup_stop_id} onChange={e => setFormData({ ...formData, pickup_stop_id: e.target.value })}>
                                    <option value="">Pickup Point</option>
                                    {selectedRoute.stops?.map(s => <option key={s.id} value={s.id}>{s.stop_name}</option>)}
                                </select>
                                <select required className="border rounded px-3 py-2" value={formData.drop_stop_id} onChange={e => setFormData({ ...formData, drop_stop_id: e.target.value })}>
                                    <option value="">Drop Point</option>
                                    {selectedRoute.stops?.map(s => <option key={s.id} value={s.id}>{s.stop_name}</option>)}
                                </select>
                            </div>
                            <input placeholder="Seat Number (Optional)" className="w-full border rounded px-3 py-2" value={formData.seat_number} onChange={e => setFormData({ ...formData, seat_number: e.target.value })} />
                        </>
                    )}

                    <div className="flex justify-end space-x-3 mt-4">
                        <button type="button" onClick={onClose} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded">Cancel</button>
                        <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Allocate</button>
                    </div>
                </form>
            </div>
        </div>
